...
#Función para obtener los tweets de la BBDD
...
#Funión para obtener las entidades y relaciones de los tweets
def get_entities_and_relations(tweets):
    entity_pairs = []
    relation_pairs = []
    num = len(tweets)
    for i in tqdm(range(num)):
        print('', end='\r')
        l = split_in_sentences(tweets[i])
        for txt in l:
            if txt != '':
...
def delete_empty_entities_and_relations(list_entities, list_relation):
...
# OBTENER LAS RELACIONES Y LAS ENTIDADES
entity_pairs = []
relation_pairs = []
entity_pairs,relation_pairs = get_entities_and_relations(tweets)
entity_pairs,relation_pairs = delete_empty_entities_and_relations(entity_pairs,relation_pairs)
...
# print('nodo 1:')
# print(pd.Series(source).value_counts()[:4])
# print('nodo 2:')
# print(pd.Series(target).value_counts()[:4])
# print('relaciones:')
...
plt.title('Graph Knowledge of SDGs')
